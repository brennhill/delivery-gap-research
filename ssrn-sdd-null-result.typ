...
#par(first-line-indent: 0pt)[
  #text(size: 10.5pt)[
*Context.* Spec-driven development (SDD) tools claim that writing specifications before implementation reduces defects, prevents rework, and improves code quality. These claims lack empirical evidence. No production-scale SDD tool data is publicly available.
*Objective.* We provide the first large-scale empirical test of five hypotheses derived from SDD vendor claims, using specification artifacts in open-source pull requests --- the closest available proxy --- scored on the same quality dimensions SDD tools prescribe.
*Method.* We analyze 88,052 pull requests across 119 open-source repositories (103 with SZZ defect tracing coverage), using within-author fixed-effects estimation, propensity score matching, and ten robustness checks.
*Results.* None of the five hypotheses are supported at a level that would justify the vendor claims. The unadjusted association between specifications and defects is reversed (specifications accompany more defects, not fewer); after within-author controls, the most likely interpretation is confounding by indication. Specification quality has no meaningful effect on rework (_p_ = 0.827). Specifications do not constrain AI-generated code scope (_p_ = 0.997). One hypothesis (specification quality reduces defects) shows a small signal (_p_ = 0.016) that does not survive robustness checks, predicting only 8 fewer defects per 1,000 pull requests per 10-point quality improvement.
*Conclusion.* Specification artifacts proxy for task complexity, not quality improvement.
  ]
]
...
These claims rest on a plausible causal chain: clearer requirements → less ambiguity → fewer defects. However, none of the marketing materials, documentation, or blog posts accompanying these tools cite empirical evidence for the claimed outcomes. No empirical evidence is cited in support of these assertions. No study has tested SDD's quality claims on production-scale data prior to this paper.

This paper provides the first large-scale empirical test. We analyze 88,052 merged pull requests across 119 open-source repositories, after excluding 12,195 bot-authored PRs. Defects are traced to their introducing commits using the SZZ algorithm. We use within-author fixed-effects estimation to control for developer-level confounding, and derive five hypotheses directly from vendor claims.
...
#par(first-line-indent: 0pt)[
  *Pooled:* Raw rates and Fisher's exact test. This is the analysis that unadjusted claims would cite.
]
...
#figure(
  table(
    ...
    [Pooled (Fisher's exact)], [OR = 1.202], [< 0.001], [Spec'd PRs have _higher_ defect rate],
    [Controlled (logit, no repo FE#super[†])], [0.067], [0.006], [Effect persists with size controls],
    [Within-author (LPM)], [+0.014 \[+0.005, +0.024\]], [0.003], [+1.4pp _increase_ in defect rate],
  ),
...
*H1 is not supported.* The pooled comparison is the unadjusted analysis: spec'd PRs have a 20.2% higher odds of introducing a defect than unspec'd PRs. But this comparison is confounded --- developers who write specs are different from those who don't, and the repos that encourage specs are different from those that don't. The controlled logistic regression adds size controls (repository fixed effects caused a singular matrix and were dropped). The effect persists.

The within-author estimate is the most credible. It compares the _same developer's_ spec'd PRs to their own unspec'd PRs, eliminating all time-invariant author-level confounding --- skill, experience, coding style, and organizational context are held constant because both the "treatment" and "control" come from the same person. Within-author, spec'd PRs are associated with a 1.4 percentage-point _increase_ in defect-introduction rates (_p_ = 0.003, OR = 1.13, Cohen's _d_ = 0.07). The direction is opposite to the vendor claim at every level of analysis.

This is consistent with confounding by indication: harder, riskier tasks receive specifications _and_ introduce more defects. A developer rationally invests specification effort in proportion to task complexity. The specification does not cause the defects --- the task difficulty that motivates the specification also produces the defects. When JIT risk features are added as controls (Section~5.6), the within-author spec coefficient drops 55% and loses significance (_p_ = 0.229), confirming that measurable task complexity explains most of the association.
...
#figure(
  table(
    columns: 4,
    table.header[Method][Coefficient][_p_-value][Interpretation],
    [Controlled (logit + repo FE)], [−0.005], [0.045], [Quality predicts fewer defects],
...
*H3 is weakly supported on a biased subsample.* The within-author coefficient is statistically significant (_p_ = 0.016) but substantively small: a 10-point improvement in specification quality (on the 0--100 scale) predicts a 0.8 percentage-point reduction in defect rate. This is the one result in the SDD-predicted direction. However, three caveats limit its interpretation. First, it is identified from 357 authors on 5.9% of the data, in a subsample non-randomly selected for having substantial specification content. Second, when restricted to the four quality dimensions with validated human--LLM agreement (Section~5.6), the effect weakens (_p_ = 0.075) while the unvalidated dimensions strengthen --- suggesting the signal may reflect description length or task complexity rather than specification quality (Section~5.6). Third, the effect size is small enough that even if causal, it would not justify the specification effort required to achieve it.
...
#figure(
  ...
  caption: [Incremental validity of specifications beyond JIT features.],
)

#text(size: 9pt)[#super[†] Scored subset only (5,192 PRs). JIT-only Pseudo~_R_#super[2] on same subset = 0.095.]

Spec presence is statistically significant when added to the JIT model (_p_ = 0.003) but the increment is substantively negligible: Δ Pseudo~_R_#super[2] = 0.0002, and AIC decreases by only 7 points. Spec quality adds a marginally significant increment (_p_ = 0.059) on a biased subsample. In both cases, the incremental predictive power is too small to matter in practice --- JIT features account for nearly all the predictive power.
...
All coefficients in the defects column are negative (directionally protective) but extremely small. To put the magnitudes in plain language: the largest coefficient is −0.001 (scope boundaries). This means that scoring the maximum possible improvement on scope boundaries --- moving from 0 to 100 on the rubric --- would predict 0.1 fewer percentage points of defects. In a codebase with a 12% base defect rate, that is a reduction from 12.0% to 11.9%. No individual quality dimension produces a practically meaningful reduction in defects. The rework column is even starker: every coefficient rounds to zero.

Three dimensions show marginal statistical significance for defects (scope boundaries _p_ = 0.027, data contracts _p_ = 0.037, behavioral specificity _p_ = 0.020), but none survive Bonferroni correction#footnote[With 7 simultaneous tests at α = 0.05, the Bonferroni-corrected threshold is _p_ < 0.007.] (threshold: _p_ < 0.007 for 7 tests). Statistical significance at these magnitudes reflects the large sample size, not a meaningful effect. Even if these tiny coefficients reflected real causal relationships, they would be too small to justify the specification effort required to achieve them.
...
At the repository level, specification rate has no relationship with either defect rate (_ρ_ = −0.11, _p_ = 0.27) or rework rate (_ρ_ = −0.05, _p_ = 0.64). Neither correlation is significant. An OLS regression controlling for mean PR size yields a spec rate coefficient of −0.060 (_p_ = 0.13), which is not significant.
...
The pattern is consistent across subgroups. For human-authored PRs, specs are associated with _more_ defects and _more_ rework --- the confounding-by-indication pattern. For AI-tagged PRs, the coefficients are positive but not significant (_p_ = 0.42 for defects, _p_ = 0.40 for rework). The null holds whether a repo has zero AI adoption, moderate adoption, or high adoption.
...
Spec'd PRs take substantially longer to merge (median 27.2 hours vs. 5.8 hours for unspec'd), but this additional review time does not translate into fewer defects --- the same confounding pattern. AI-tagged PRs receive less review scrutiny despite a higher base defect rate (16.7% vs. 12.2%), a separately concerning finding.
...
SDD vendors conflate _directing_ the AI (telling it what to build) with _ensuring quality_ (fewer defects, less rework). Specifications may be effective at direction --- a structured task description is better than a vague prompt. But direction is not quality. An agent faithfully executing a spec will reproduce every gap, ambiguity, and wrong assumption in that spec. The specification tells the AI _what_ to build. It does not tell the AI _what it forgot to specify_.
