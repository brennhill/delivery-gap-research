...
Böckeler #cite(<fowler2025>) defines spec-driven development as "writing a 'spec' before writing code with AI" where "[t]he spec becomes the source of truth for the human and the AI." The GitHub Copilot Academy frames it more strongly: "Specifications don't serve code --- code serves specifications" #cite(<copilotacademy2025>). The movement gained momentum in 2025 with the release of GitHub's Spec Kit (open-source, v0.0.72 as of this writing) and Amazon's Kiro IDE. Böckeler, writing on Martin Fowler's site, examined three tools pursuing the approach #cite(<fowler2025>). The methodology is positioned as an antidote to "vibe coding" --- the practice of iteratively prompting AI without formal requirements --- which practitioners associate with scope creep, rework, and defects.
...
These claims rest on a plausible causal chain: clearer requirements → less ambiguity → fewer defects. However, none of the marketing materials, documentation, or blog posts accompanying these tools cite empirical evidence for the claimed outcomes. The assertions are presented as self-evident truths rather than testable hypotheses. As far as we can determine, no study has tested SDD's quality claims on production-scale data prior to this paper.
...
We collected pull request data from 119 open-source repositories spanning 14 programming languages. Repositories were selected to represent large, active projects with diverse contribution patterns. Data was collected between March 28 and April 3, 2026, with a 365-day lookback window per repository. The resulting dataset spans April 2025 through early April 2026 (p1--p99: April 8, 2025 to April 1, 2026), totaling 100,247 PRs. After excluding 10,648 bot-authored PRs (dependabot, renovate, and 22 other automated accounts identified by username pattern and bot flags), 89,599 PRs from human and AI-assisted authors remain.
...
#figure(
  table(
    ...
    [SZZ-covered repos], [103], [16 failed due to clone timeouts],
...
The sample spans 17 programming languages, with Python (31 repos, 26%), TypeScript (26, 22%), Go (17, 14%), and Rust (13, 11%) predominating. Specification rates are highly skewed: the median repository has 0% spec'd PRs, while the 75th percentile is 15.3%. AI tag rates are similarly concentrated: the median is 0%, with a 75th percentile of 0.9%.
...
Five additional analyses test whether the null result is an artifact of our primary measures or insufficient controls.
...
SDD tools are marketed primarily for AI-assisted workflows. We test whether the null result holds across human-only PRs, AI-tagged PRs (excluding bots), and repos stratified by AI adoption rate. We exclude 10,648 bot PRs (dependabot, renovate, and other automated authors) from all analyses (including the main results above).
...
*We detect specification artifacts, not specification processes.* Our classifier flags linked issues, structured descriptions, and referenced design documents. A linked Jira ticket created to satisfy a process requirement counts the same as a detailed design document written after iterative refinement. If the benefit comes from the _thinking_ rather than the _artifact_, our measure is too coarse. Two things partially address this. First, the within-author design compares the same developer's spec'd and unspec'd work, controlling for that person's consistent habits. Second, H3 and H4 test quality _within_ spec'd PRs on seven SDD-aligned dimensions. If deeper thinking produced better artifacts, those artifacts should predict better outcomes. They do not. The remaining objection --- that process depth helps through a mechanism invisible in the artifact itself --- is difficult to test with any observational design.
