#!/usr/bin/env python3
"""Score human engagement signals in spec text using Claude Haiku.
...
Usage:
    python3 score-engagement.py --repo cli-cli     # score one repo
    python3 score-engagement.py --test              # test on known examples
    python3 score-engagement.py                     # score all repos
"""
...
def score_engagement(title: str, body: str, model: str = "claude-haiku-4-5-20251001") -> dict:
    spec_text = f"Title: {title}\n\n{body[:4000]}"
    prompt = ENGAGEMENT_PROMPT + spec_text
    try:
        if _has_api_key():
...
def run_test():
    ...
    for group_label, examples in [("KNOWN HUMAN", KNOWN_HUMAN), ("KNOWN AI", KNOWN_AI)]:
        print(f"\n--- {group_label} ---\n")
        for ex in examples:
            result = score_engagement(ex["title"], ex["body"])
            if "error" in result:
...
def score_repo(repo_slug: str, dry_run: bool = False) -> list[dict]:
    ...
    for i, spec in enumerate(specs):
        ...
        try:
            scores = score_engagement(spec["title"], spec["body"])
...
def main():
    parser = argparse.ArgumentParser(description="Score human engagement via LLM")
