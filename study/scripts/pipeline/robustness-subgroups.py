...
# ── Bot exclusion ─────────────────────────────────────────────────────
# Combine is_bot_author flag with name-pattern matching for bots
# that slip through (dependabot, renovate, etc.)

BOT_PATTERNS = [
    "dependabot", "renovate", "greenkeeper", "snyk", "mergify",
    "codecov", "denobot", "vitess-bot", "ti-chi-bot", "pwshbot",
    "k8s-infra-cherrypick-robot", "medplumbot", "refine-bot",
    "lobehubbot", "qdrant-cloud-bot", "vercel-release-bot",
    "nextjs-bot", "grafana-delivery-bot", "grafana-pr-automation",
    "github-actions", "scheduled-actions", "copilot-swe-agent",
    "robobun", "promptfoobot", "langchain-model-profile-bot",
    "n8n-assistant", "mendral-app",
]

df["is_bot_flag"] = df["f_is_bot_author"].fillna(False).astype(bool)
bot_pattern = "|".join(BOT_PATTERNS)
df["is_bot_name"] = df["author"].str.lower().str.contains(
    bot_pattern, case=False, na=False
)
df["is_bot"] = df["is_bot_flag"] | df["is_bot_name"]
